...
def register_pull(app: typer.Typer):
    ...
    def pull(
        ...
        Examples:
          observal pull my-agent --ide claude-code --no-prompt
          observal pull my-agent --ide kiro --no-prompt --scope user
          observal pull my-agent --ide cursor --no-prompt --dry-run
          observal pull my-agent --ide kiro --no-prompt --env API_KEY=sk-123 --env SECRET=abc
        """

...
def _row_to_trace(r: dict) -> Trace:
    return Trace(
        ...
        input=_parse_json(redact_secrets(r["input"]) if r.get("input") else r.get("input")),
        output=_parse_json(redact_secrets(r["output"]) if r.get("output") else r.get("output")),
        tags=r.get("tags", []),
...
def _row_to_span(r: dict) -> Span:
    ...
    return Span(
        ...
        input=_parse_json(redact_secrets(r["input"]) if r.get("input") else r.get("input")),
        output=_parse_json(redact_secrets(r["output"]) if r.get("output") else r.get("output")),
        error=_parse_json(redact_secrets(r["error"]) if r.get("error") else r.get("error")),
        start_time=r.get("start_time", ""),
        end_time=r.get("end_time"),
